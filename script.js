document.addEventListener("DOMContentLoaded", () => {
    ...
    // ---- Reactive State Configuration Layers ----
    let currentMode = "shows";      
    let apiDataResults = [];        
    let isDarkMode = true;
    let savedCount = 0;

    // Trigger immediate feed populating on load
    fetchLiveTVMazeData("");

    // ---- Asynchronous Endpoint Channel Link ----
            ...
            apiDataResults = jsonResults.map(item => {
                if (currentMode === "shows") {
                    ...
                    return {
                        ...
                        genres: showData.genres ? showData.genres.join(', ') : 'General Entertainment',
                        rating: showData.rating?.average ? `⭐ ${showData.rating.average}/10` : 'No rating recorded'
                    };
                } else {
                    ...
                    return {
                        ...
                        name: actorData.name,
                        summary: actorData.birthday ? `Born: ${actorData.birthday}` : "Professional artist profile card.",
                        img: actorData.image ? actorData.image.medium : `https://robohash.org/${encodeURIComponent(actorData.name)}?set=set4`,
                        genres: actorData.country ? `Origin: ${actorData.country.name}` : 'International Artist',
                        rating: 'Artist Profile'
                    };
                }
            });
    ...
    // ---- Build Layout Elements into Document Grid ----
    function renderDisplayGridFeed() {
        ...
        apiDataResults.forEach((item, index) => {
            ...
            layoutCard.innerHTML = `
                <div class="item-click-trigger" data-index="${index}">
                    ...
                    <p class="text-xs font-semibold text-indigo-400 mb-2">${item.rating}</p>
            ...
            // Setup direct modal detail overlay triggers
            layoutCard.querySelector(".item-click-trigger").addEventListener("click", () => openDetailModalOverlay(item));
            
            // Setup internal tracking save action triggers
            layoutCard.querySelector(".save-action-btn").addEventListener("click", (e) => {
                e.stopPropagation();
                savedCount++;
                if (watchlistCount) watchlistCount.textContent = savedCount;
            });
        ...
        modalBodyContent.innerHTML = `
            <div class="flex flex-col sm:flex-row gap-6">
                ...
                <div class="flex flex-col justify-center">
                    ...
                    <span class="text-xs font-bold text-indigo-400 uppercase tracking-widest mb-3 block">${item.genres}</span>
    ...
    showModeBtn?.addEventListener("click", () => {
        currentMode = "shows";
        showModeBtn.className = "flex-1 text-center text-sm font-medium py-2 rounded-lg bg-indigo-600 text-white shadow-md transition-all";
    ...
    actorModeBtn?.addEventListener("click", () => {
        currentMode = "actors";
        actorModeBtn.className = "flex-1 text-center text-sm font-medium py-2 rounded-lg bg-indigo-600 text-white shadow-md transition-all";
    ...
    // Random query shuffle tool engine [Fulfills Extra UX Brief Components]
    shuffleBtn?.addEventListener("click", () => {
        const alphabet = "bcdefghijklmnoprstvw";
        const randomChoice = alphabet[Math.floor(Math.random() * alphabet.length)];
        if (searchInput) searchInput.value = randomChoice;
        fetchLiveTVMazeData(randomChoice);
    });

    // Theme Toggle Handler Engine
